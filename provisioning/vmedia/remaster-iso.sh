...
# If no IPXE file was provided, or if config-file is used directly:
if [[ -z "$CMDLINE_PARAMS" ]] && [[ -n "$CONFIG_FILE" ]]; then
    echo "    Translating YAML config ($CONFIG_FILE) into kernel arguments..."
    CMDLINE_PARAMS="$(python3 "$YAML_CONVERTER" "$CONFIG_FILE" --mode "$MODE")"
fi
...
echo "    Injected kernel parameters:"
echo "    >>> $CMDLINE_PARAMS <<<"
...
try:
    ...
    sys.exit(1)
" "${EXTRACT_DIR}/boot/grub2/grub.cfg" "$GRUB_TIMEOUT" "$MODE_UPPER"
